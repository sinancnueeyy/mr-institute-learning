...
import { charityRepository } from '../../repositories/operations';
import type { CharityApplication } from '../../types/operations';
import { Eye, Download, Check, X, FileText, Phone } from 'lucide-react';
import { toast } from 'react-hot-toast';

export default function CharityApplications() {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const res = await charityRepository.getAll();
  ...
  const handleStatusChange = async () => {
    ...
    try {
      await charityRepository.update(statusAction.id, { 
  ...
  const filteredApps = applications.filter(app => {
    const matchesSearch = app.applicantName.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          ...
                          app.requestType.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const columns = [
    { 
      ...
      header: 'Applicant / Guardian',
      cell: (item: CharityApplication) => (
        <div>
    ...
    { 
      ...
      header: 'Request Type',
      cell: (item: CharityApplication) => <span className="text-sm font-medium">{item.requestType}</span>
    },
    { 
      accessorKey: 'submittedAt', 
      header: 'Date',
      cell: (item: CharityApplication) => <span className="text-sm">{new Date(item.submittedAt).toLocaleDateString()}</span>
    },
    { 
      accessorKey: 'status', 
      header: 'Status',
      cell: (item: CharityApplication) => <StatusBadge status={mapStatus(item.status)} />
    },
    {
      accessorKey: 'actions',
      header: 'Actions',
      cell: (item: CharityApplication) => (
        <div className="flex items-center gap-2">
          ...
          {item.status === 'pending' && (
            <Button variant="ghost" size="sm" onClick={() => promptStatusChange(item.id, 'approved')} className="text-success hover:text-success hover:bg-success/10">
  ...
  return (
    <FadeIn className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Charity Requests</h1>
          <p className="text-text-secondary">Manage community charity and support applications.</p>
        </div>
      ...
      <FilterPanel 
        ...
        searchPlaceholder="Search by name, contact or type..."
        statusOptions={[
      ...
      <div className="bg-white rounded-xl border border-border overflow-hidden">
        <DataTable 
          ...
          isLoading={loading}
          emptyMessage="No charity requests found matching your criteria."
        />
      </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-text-secondary flex items-center gap-1 mt-1">
                  <Phone className="w-4 h-4" /> {selectedApp.contact}
                </p>
              </div>
            ...
            <div className="bg-surface p-6 rounded-xl border border-border space-y-4">
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="font-semibold text-text-primary">{selectedApp.requestType}</p>
                </div>
                <div>
              ...
              <div className="pt-4 border-t border-border border-dashed">
                <p className="text-sm text-text-muted mb-2">Description / Need</p>
                <p className="text-sm text-text-primary bg-white p-4 rounded border border-border leading-relaxed">
                  {selectedApp.description}
                </p>
              </div>

              <div className="pt-4 border-t border-border border-dashed">
                <p className="text-sm text-text-muted mb-2">Supporting Documents</p>
                {selectedApp.documents?.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {selectedApp.documents.map((doc: string, idx: number) => (
                      <DocumentPreview key={idx} url={doc} title={`Support Document ${idx + 1}`} />
                    ))}
                  </div>
                ) : (
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                {selectedApp.status === 'pending' && (
                  <>
                    <Button onClick={() => promptStatusChange(selectedApp.id, 'approved')} className="bg-success hover:bg-success/90 text-white">
                ...
                <Button variant="outline">
                  <FileText className="w-4 h-4 mr-2" /> Request More Documents
                </Button>
              </div>
            </div>

            <div className="space-y-4 pt-4 border-t border-border">
              <h3 className="font-bold text-text-primary">Timeline</h3>
              <WorkflowTimeline 
                events={[
                  {
                    id: '1',
